...
public class MyFavouriteQuestions extends BaseFragment implements IQuestionOperationsListener, ISendingMessagesListener, IReportItemListener, IAddAnswer {
...
//        mCustomLoadingDialog = new CustomLoadingDialog(mContext);
        ...
        loadQuestions(Constants.favQuestionUrl);
    }
    ...
    public void onFavouriteClicked(int position) {
        if (mSharedPrefManager.getLoginStatus()) {
            String questionID = questionsList.get(position).getQuestion_id();
            questionsList.remove(position);
            mQuestionAdapter.notifyDataSetChanged();
            addToFavourite(Constants.addToFavouriteUrl, questionID);
        } else {
    ...
    private void parseQuestionsResponse(String response) {
        try {
            ...
            if (questionsList.isEmpty()) {
                ...
                noDataTextView.setText("لا يوجد أسئلة مفضلة");
            } else {
