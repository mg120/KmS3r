...
public class MyFavouriteOffers extends BaseFragment implements IOffersOperationsListener, ISendingMessagesListener {
    Context mContext;
    ...
    protected void initializeComponents() {
        ...
        loadOffers(Constants.favOffersUrl);
    }
    ...
    public void onFavouriteClicked(int position) {
        if (mSharedPrefManager.getLoginStatus()) {
            String offerId = offersList.get(position).getOffer_id();
            offersList.remove(position);
            mOffersAdapter.notifyDataSetChanged();
            addToFavourite(Constants.addToFavouriteUrl, offerId);
        } else {
    ...
    public void onShareClicked(int position) {
        Utils.Share(mContext, getString(R.string.offer_share), offersList.get(position).getOffer_title());
        ...
        try {
            ...
            if (jsonObject.optBoolean("response")) {
                if (jsonObject.has("asks")) {
                    JSONObject offersObject = jsonObject.optJSONObject("asks");
                    ...
                    for (int i = 0; i < offersArray.length(); i++) {
                        ...
                        System.out.println("id : "+offersModel.getOffer_id());
            ...
            if(offersList.isEmpty()){
                ...
                noDataTextView.setText("لا يوجد عروض مفضلة");
            }
            else {
